...
import com.Zetin.databases.EdgesDbOpenHelper;
import com.Zetin.databases.PoisDbOpenHelper;
import com.Zetin.R;
...
public class MainActivity extends ListActivity implements SensorEventListener,OnClickListener{
	...
	/**
	 * 스마트폰이 블루투스를 지원하지 않거나 활성화되어있지 않으면 바로 종료한다
	 */
	 ...
	 */
	@Override
	public boolean onPrepareOptionsMenu(Menu menu) { //메뉴
		menu.clear();
		if(MODE.equals("NAVI")){
	...
	/**
	 * 만약 Pause 상태에 있다가 Activity가 다시 활성화 되어 Resume 상태로 가게 된다면 시스템은 onResume 함수를 호출
	 */
	...
	/**
	 * 다른 Activity나 컴포넌트로 덮어져서 일시적으로 사용할 수 없을 때 Pause 상태로 들어감
	 */
	...
	/**
	 * 생성될때에는 onCreate라던 반대로 Activity가 종료될 때에는 onDestroy 함수가 콜백된다
	 */
